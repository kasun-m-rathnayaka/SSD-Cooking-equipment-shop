...
const postSellProduct = async (req, res) => {
    const { id, title, category, cost, sellingprice, qty, profit, date } = req.body

    try {
        const response = await productModel.create({ id, title, category, cost, sellingprice, qty, profit, date })
        res.status(200).json(response)
    } catch (error) {
